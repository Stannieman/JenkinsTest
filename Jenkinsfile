  ...
  stages {
	...
	stage('Pack') {
	  ...
	  stage('Publish') {
		  ...
		  steps {
			  bat 'pwsh.exe -Command "& \'%EXECUTABLE_DOTNET_CORE_2_0%\' nuget push -s https://staging.nuget.org -ss https://nuget.smbsrc.net -k %NUGET_API_KEY% ((Get-ChildItem -Recurse *.nupkg | Select-Object -ExpandProperty FullName) -Match \'\\\\bin\\\\Release\\\\\')"'
		  }
	  }
  }
	  /*post {
